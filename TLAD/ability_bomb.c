...
int sub_4747(int iParam0, unknown uParam1)
{
    if (IS_PLAYER_PLAYING( sub_632() ))
    {
        if (IS_SCREEN_FADED_IN())
        {
            ...
            {
                if (((g_U10499) AND (uParam1)) || ((iParam0 == 4) || ((iParam0 == 5) || (IS_PLAYER_FREE_FOR_AMBIENT_TASK( sub_632() )))))
                {
                    if (NOT sub_4843())
                    {
...
int sub_6120()
{
    ...
    {
        if ((sub_6154()) AND (ARE_WIDESCREEN_BORDERS_ACTIVE()))
        {
            return 1;
        }
