    ...
    while (true)
    {
        ...
        if (IS_PLAYER_PLAYING( sub_1227() ))
        {
            ...
            if (DOES_OBJECT_EXIST( l_U22 ))
            {
                ...
                SET_OBJECT_DYNAMIC( l_U22, 0 );
                if ((NOT (sub_3031( l_U22 ))) AND (IS_WORLD_POINT_WITHIN_BRAIN_ACTIVATION_RANGE()))
                {
...
int sub_3611()
{
    ...
    if ((HAS_MODEL_LOADED( sub_3667() )) AND ((REQUEST_AMBIENT_AUDIO_BANK( "SCRIPT_AMBIENT\TOLLBOOTH" )) AND ((HAVE_ANIMS_LOADED( "AMB@TOLLBOOTH" )) AND ((HAS_MODEL_LOADED( -1174154857 )) AND (HAS_MODEL_LOADED( -1295524692 ))))))
    {
        return 1;
    }
    return 0;
